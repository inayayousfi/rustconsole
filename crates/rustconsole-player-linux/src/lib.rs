...
pub use audio_output::{
    AudioPlaybackQueue, AudioPlaybackSnapshot, SdlAudioOutput, run_sdl_audio_proof,
};
#[cfg(target_os = "linux")]
...
impl StreamAudioDecoder {
    fn event(&mut self, event: AudioPlaybackEvent) -> Vec<DecodedAudioEvent> {
        match event {
            AudioPlaybackEvent::Packet(packet) => self.packet(packet),
            AudioPlaybackEvent::Missing {
                ...
                output.push(DecodedAudioEvent::Samples(AudioSamples {
...
pub fn stream_quic_video(
    ...
    next_input: impl FnMut() -> Option<rustconsole_protocol::InputEvent> + Send + 'static,
    ...
    let mut audio_decoder = StreamAudioDecoder::default();
...
mod tests {
    ...
    fn stream_decoder_emits_reset_and_real_opus_samples() {
        ...
        let events = decoder.event(AudioPlaybackEvent::Packet(encoded_packet(1, 0)));
        assert!(matches!(
            events.first(),
        ...
        assert_eq!(samples.format.sample_rate, 48_000);
        assert_eq!(samples.format.channels, 2);
        assert_eq!(samples.interleaved.len(), 336);
        assert!(samples.interleaved.iter().any(|sample| *sample != 0.0));
    }
    ...
    fn stream_decoder_bounds_missing_audio_and_contains_codec_failure() {
        ...
        assert_eq!(silence.interleaved.len(), 4 * 480 * 2);
        assert!(silence.interleaved.iter().all(|sample| *sample == 0.0));

        let mut invalid = encoded_packet(3, 0);
        invalid.payload.clear();
        let failed = decoder.event(AudioPlaybackEvent::Packet(invalid));
        assert!(matches!(
            failed.last(),
